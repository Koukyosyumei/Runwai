...
-/
theorem get_mem_of_eq {Γ: Env.TyEnv} {x: String} {τ: Ast.Ty}:
  Env.getTy Γ x = τ → (x, τ) ∈ Γ := by {
    unfold Env.getTy
...
-/
theorem get_mem_impl_some {Γ: Env.TyEnv} {x: String} {τ: Ast.Ty} (hmem: (x, τ) ∈ Γ):
  (Env.getTy Γ x).isSome  := by {
    unfold Env.getTy
    cases b: List.find? (fun x_1 ↦ decide (x_1.1 = x)) Γ with
    | none => {
      simp at b
